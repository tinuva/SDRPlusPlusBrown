...
public:
	...
	NOTIFY_INLINE auto get_elapsed_time() { return currentTimeMillis() - this->creation_time; }
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->creation_time =currentTimeMillis();
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			auto icon = current_toast->get_icon();
...
//			SetNextWindowPos(ImVec2(0, 0), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
